...
The following code computes the length of the intervals in number of days and this is stored in "time.intervals" which will be used by the "process.data"" function.
```{r}
...
* N  not reproductive during breeding season or pup dies (only observable if female seen with juvenile)
...
In addition to the natality state, the area in which they are seen is constructed. For each sea lion, the number of re-sightings in each area (one or two) are counted and the majority is used to assign the area.  
...
area_mat=array(NA,dim=c(length(bybrand),length(levels(Alive$occasion)),2))
equaln=0
for(i in 1:length(bybrand))
{
  msch[i,paste(bybrand[[i]][1,"cohort"],"01",sep="-")]="Y1"
  ...
  for(occ in colnames(msch))
  {
    ...
    if(nrow(resightings)>0)
    {
      ...
      if(msch[i,occ]!="0") msch[i,occ]=paste(msch[i,occ],area,sep="")
    }
  }
}
cat("area 1 ", sum(area_mat[,,1]>0 & area_mat[,,2]==0,na.rm=T))
cat("area 2 ", sum(area_mat[,,2]>0 & area_mat[,,1]==0,na.rm=T))
cat("total ", sum(area_mat[,,2]>0 | area_mat[,,1]>0,na.rm=T))
cat("equal ",sum(area_mat[,,2]>0 & area_mat[,,1]>0 &area_mat[,,1]==area_mat[,,2],na.rm=T))
cat("total check ",sum(msch!="0")-nrow(msch))
```


Next create the dataframe sl which contains the capture history character string (ch).
```{r}
#create data frame
sl=data.frame(ch=apply(msch,1,paste,collapse=","),stringsAsFactors=F)
chmat=sapply(sl$ch,function(x) paste(substr(strsplit(x,",")[[1]],1,1),collapse=""))
...
for(i in 1:length(sl$ch))
{
  ...
  for(year in 1987:lastyear)
  {
    j=j+1
    if(year==lastyear)
      ch=strsplit(substr(chmat[i],(j-1)*8+1,(j-1)*8+7),"")[[1]]
    else    
    ch=strsplit(substr(chmat[i],(j-1)*8+1,j*8),"")[[1]]
    ch=ch[!ch%in%c("0","U")]
...
{
  cat("\n")
  print(table(zz[,seq(i,length(strsplit(sl$ch[1],",")[[1]]),8)]))
}
