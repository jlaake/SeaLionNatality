...
Sea lions are observed throughout the breeding season and the following reproductive status observation codes can be recorded when appropriate:
...
Those 3 codes refer to states P,B,and N respectively as described below.

The reproductive status observation codes depend on the state of the female sea lion. For example, if a female is pregnant or seen with a pup then she is reproductive that year and if she is seen with a juvenile and not with a pup then she was reproductive a previous year but not that year. We define the following reproductive states for the model:
...
* L : lactating and pregnant (not observable and only occurs between breeding seasons)

A typical set of state transitions would be for the female to remain in state Y until its first pregnancy when it transitions to P and then to B in the season when the pup is born. If the pup dies during the breeding season the female transitions to state N. If a female that was prviously reproductive does not get pregnant then then she is in state N for the entire breeding season.  
...
* B to L : had pup at end of breeding season and becomes pregnant so she is both lactating and pregnant
...
* L to B : from pregnant and lactating to giving birth for interval 8 to 1
* L to P : from pregnant and lactating to remaining pregnant for interval 8 to 1; by subtratction
...
dp=process.data(sl,begin.time=1987,model="mvmscjs",time.intervals=ti,strata.labels=list(nstate=c("Y","N","P","B","L","u")),accumulate=FALSE)
```
...
ddl$p$P=as.integer(ddl$p$stratum%in%c("L","P"))
ddl$p$cohort=NULL
ddl$p$Cohort=NULL
...
ddl$Phi$L=as.integer(ddl$Phi$stratum=="L")
ddl$Phi$Cohort=NULL
ddl$Phi$area=NULL
```
...
ddl$Psi$BtoP=as.numeric(ddl$Psi$stratum=="B")*as.numeric(ddl$Psi$tostratum=="L")*as.numeric(ddl$Psi$interval=="7")
...
ddl$Psi$PtoB=as.numeric(ddl$Psi$stratum%in%c("P","L"))*as.numeric(ddl$Psi$tostratum=="B")*as.numeric(!ddl$Psi$interval%in%c("7"))
...
The pregnant state P can only transition to B during intervals 1 to 5 or 8. If still in P at interval 6, then it is forced to B by setting that transition probability to 1.
```{r}
...
```{r}
# L stratum (pregnant&lactating - not observable)
# L must go to P or B from interval 8 to 1.
ddl$Psi$fix[ddl$Psi$stratum=="L"&ddl$Psi$tostratum!="L"]=0
ddl$Psi$fix[ddl$Psi$stratum=="L"&ddl$Psi$tostratum=="B"&ddl$Psi$interval==8]=NA
ddl$Psi$fix[ddl$Psi$stratum=="L"&ddl$Psi$tostratum=="P"&ddl$Psi$interval==8]=1
ddl$Psi$fix[ddl$Psi$stratum=="L"&ddl$Psi$tostratum=="L"&ddl$Psi$interval==8]=0
```
...
ddl$Psi$fix[ddl$Psi$stratum=="B"&!ddl$Psi$tostratum%in%c("N","B","L")]=0
ddl$Psi$fix[ddl$Psi$interval!=7 & ddl$Psi$stratum=="B"&ddl$Psi$tostratum=="L"]=0
