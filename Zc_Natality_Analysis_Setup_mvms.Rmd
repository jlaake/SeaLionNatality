...
dp=marked:::process.data(sl,begin.time=1987,model="mvmscjs",time.intervals=ti,
                strata.labels=list(nstate=c("Y","N","P","B","u"),area=c("1","2")),
                accumulate=FALSE)
```
...
For pi most variables are removed because pi is known for these data since they are all released as pups in the "Y" state.
```{r}
ddl$pi$Time=NULL
ddl$pi$Cohort=NULL
ddl$pi$cohort=NULL
...
ddl$p$N=as.integer(ddl$p$nstate=="N")
ddl$p$B=as.integer(ddl$p$nstate=="B")
ddl$p$P=as.integer(ddl$p$nstate=="P")
ddl$p$cohort=NULL
ddl$p$Cohort=NULL
...
ddl$Phi$N=as.integer(ddl$Phi$nstate=="N")
ddl$Phi$Y=as.integer(ddl$Phi$nstate=="Y")
ddl$Phi$P=as.integer(ddl$Phi$nstate=="P")
ddl$Phi$Cohort=NULL
ddl$Phi$area=NULL
```
...
ddl$Psi$NtoP=as.numeric(ddl$Psi$nstate=="N")*as.numeric(ddl$Psi$tonstate=="P")*as.numeric(ddl$Psi$interval=="7")
...
ddl$Psi$BtoB=as.numeric(ddl$Psi$nstate=="B")*as.numeric(ddl$Psi$tonstate=="B")
ddl$Psi$PupS=ddl$Psi$BtoB
# B to P can only occur for interval 7; exclude early years when all have to be in state Y
ddl$Psi$BtoP=as.numeric(ddl$Psi$nstate=="B")*as.numeric(ddl$Psi$tonstate=="P")*as.numeric(ddl$Psi$interval=="7")
# Y to P is not specified for intervals but all other than interval 7 are fixed to 0 below
ddl$Psi$YtoP=as.numeric(ddl$Psi$nstate=="Y")*as.numeric(ddl$Psi$tonstate=="P")
# P to B can occur in intervals 8,1 to 5
#ddl$Psi$PtoB=ddl$Psi$P*ddl$Psi$toB*as.numeric(!ddl$Psi$interval%in%c("6","7"))
ddl$Psi$PtoB=as.numeric(ddl$Psi$nstate=="P")*as.numeric(ddl$Psi$tonstate=="B")*as.numeric(!ddl$Psi$interval%in%c("7"))
# P to N for interval 7
ddl$Psi$PtoN=as.numeric(ddl$Psi$nstate=="P")*as.numeric(ddl$Psi$tonstate=="N")*as.numeric(ddl$Psi$interval=="7")
# area movement variables
ddl$Psi$move=ifelse(ddl$Psi$area!=ddl$Psi$toarea,1,0)
```
...
ddl$delta$year=factor(cut(ddl$delta$Year,c(1987,1992:2017),include.lowest=TRUE,labels=c("1987-1992",1993:2017)))
...
plabels=sort(paste(rep(sort(unique(ddl$delta$Year)),8),sort(levels(ddl$delta$interval)),sep=""))[-c(1,dp$nocc+1)]
...
ddl$delta$N=as.integer(ddl$delta$nstate=="N")
ddl$delta$B=as.integer(ddl$delta$nstate=="B")
...
fixed0=apply(zz,2,function(x) as.numeric(any(x!="0"&!x%in%c("Y1","Y2"))))
fixed0=names(fixed0[fixed0==0])
ddl$p$fix=NA
ddl$p$fix[ddl$p$time%in%fixed0]=0
plabels=sort(paste(rep(sort(unique(ddl$p$Year)),8),sort(levels(ddl$p$interval)),sep=""))[-c(1,dp$nocc+1)]
...
```{r}
ddl$Psi$fix[ddl$Psi$nstate=="B"]=NA
ddl$Psi$fix[ddl$Psi$stratum=="B1"&ddl$Psi$tostratum=="N1"]=1
ddl$Psi$fix[ddl$Psi$stratum=="B2"&ddl$Psi$tostratum=="N2"]=1
```
...
ddl$Psi$fix[ddl$Psi$nstate=="N"&!ddl$Psi$tonstate%in%c("P","N")]=0
# N can only go to P on next to last occasion 7
ddl$Psi$fix[ddl$Psi$interval!=7&ddl$Psi$nstate=="N"&ddl$Psi$tonstate=="P"]=0
```
...
# Y can only go to P
ddl$Psi$fix[ddl$Psi$nstate=="Y"&!ddl$Psi$tonstate%in%c("Y","P")]=0
# Y can only go to P at age 2 or older near end of breeding season
ddl$Psi$fix[(ddl$Psi$Age<2|ddl$Psi$interval!=7)&ddl$Psi$nstate=="Y"&ddl$Psi$tonstate=="P"]=0
# No states can go to Y
ddl$Psi$fix[ddl$Psi$nstate!="Y"&ddl$Psi$tonstate=="Y"]=0
```
...
ddl$Psi$fix[ddl$Psi$nstate=="P"&ddl$Psi$tonstate!="P"]=0
ddl$Psi$fix[ddl$Psi$nstate=="P"&ddl$Psi$tonstate=="B"&ddl$Psi$Interval%in%c(0:5)]=NA
# Force to B at 5 July (end of breeding season) 
ddl$Psi$fix[ddl$Psi$interval==6 & ddl$Psi$nstate=="P"&ddl$Psi$tonstate=="P"]=0
ddl$Psi$fix[ddl$Psi$interval==6 & ddl$Psi$stratum=="P1"&ddl$Psi$tostratum=="B1"]=1
ddl$Psi$fix[ddl$Psi$interval==6 & ddl$Psi$stratum=="P2"&ddl$Psi$tostratum=="B2"]=1
ddl$Psi$fix[ddl$Psi$interval==6 & ddl$Psi$stratum=="P1"&ddl$Psi$tostratum=="B2"]=NA
ddl$Psi$fix[ddl$Psi$interval==6 & ddl$Psi$stratum=="P2"&ddl$Psi$tostratum=="B1"]=NA
```
...
ddl$Psi$fix[ddl$Psi$nstate=="B"&!ddl$Psi$tonstate%in%c("N","B","P")]=0
ddl$Psi$fix[ddl$Psi$interval!=7 & ddl$Psi$nstate=="B"&ddl$Psi$tonstate=="P"]=0
ddl$Psi$fix[ddl$Psi$interval%in%7:8 & ddl$Psi$nstate=="B"&ddl$Psi$tonstate=="B"]=0
```
...
#######################  Fixed Delta #########################################
ddl$delta$fix=NA
ddl$delta$fix[as.character(ddl$delta$nstate)==as.character(ddl$delta$obs.nstate)]=1
ddl$delta$fix[ddl$delta$nstate=="Y"&ddl$delta$obs.nstate=="u"&ddl$delta$Age<3]=0
ddl$delta$fix[ddl$delta$nstate=="Y"&ddl$delta$obs.nstate=="u"&ddl$delta$Age>=3]=1
ddl$delta$fix[ddl$delta$nstate=="Y"&ddl$delta$obs.nstate=="Y"&ddl$delta$Age>=3]=0
ddl$delta$fix[ddl$delta$interval=="8"&ddl$delta$nstate!="Y"&ddl$delta$obs.nstate=="u"]=0
```
